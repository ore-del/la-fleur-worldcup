  ...
  return (
    <section id="claim" className="relative bg-[#0a0804] py-20 px-16">
      ...
      <form
        ...
        className="bg-white/[0.04] border-[1.5px] border-[rgba(59,112,70,0.25)] rounded-[16px] max-w-[640px] mx-auto p-[38.5px]"
      >
        <label className="block text-white/45 text-[10px] font-bold tracking-[1.5px] mb-2 uppercase">{c.bizLabel}</label>
        <div className="bg-white/[0.06] border border-[rgba(59,112,70,0.3)] rounded-[8px] h-[48px] mb-5 overflow-hidden focus-within:border-[#3b7046] focus-within:shadow-[0_0_0_2px_rgba(59,112,70,0.15)] transition-[border-color,box-shadow] duration-200">
          <input
          ...
          {c.packages.map((plan, i) => (
            <button
              ...
              className={`btn-spring rounded-[8px] h-[72px] text-left px-3 transition-[border-color,background-color] duration-200 ${
                selectedPlan === i
                  ? 'bg-[rgba(59,112,70,0.1)] border-[1.5px] border-[rgba(59,112,70,0.5)]'
        ...
        <button type="submit" className="btn-spring btn-primary-glow w-full bg-[#3b7046] text-white font-semibold text-[15px] h-[56px] rounded-[8px] hover:bg-[#6db87e]">
          {c.submit}
        </button>
      </form>
    </section>
