...
export default function ClaimSpot() {
  ...
  if (submitted) {
    return (
      <section id="claim" className="relative bg-[#0a0804] py-32 px-16 flex flex-col items-center justify-center">
        <div className="w-16 h-16 rounded-full bg-[#cb983a]/20 flex items-center justify-center mx-auto mb-6 animate-[shimmer-in_600ms_var(--spring)_both]">
  ...
  return (
    <section id="claim" className="relative bg-[#0a0804] py-20 px-16">
      ...
      >
        {/* Business name */}
        <label className="block text-white/45 text-[10px] font-bold tracking-[1.5px] mb-2 uppercase">
          Business Name
        </label>
        ...
        {/* Package */}
        <label className="block text-white/45 text-[10px] font-bold tracking-[1.5px] mb-3 uppercase">
          Choose Your Package
        </label>
        <div className="grid grid-cols-2 gap-3 mb-8">
          {[
            ...
            <button
              key={plan.label}
              type="button"
              onClick={() => setSelectedPlan(i as 0 | 1)}
            ...
            >
              <p className={`font-semibold text-[14px] ${selectedPlan === i ? 'text-[#f0c060]' : 'text-white'}`}>
                {plan.label}
              </p>
              <p className={`text-[12px] mt-1 ${selectedPlan === i ? 'text-[rgba(203,152,58,0.8)]' : 'text-white/45'}`}>
                {plan.sub}
              </p>
            </button>
          ))}
        ...
        <button
          type="submit"
          className="btn-spring btn-primary-glow w-full bg-[#cb983a] text-[#080603] font-semibold text-[15px] h-[56px] rounded-[8px] hover:bg-[#f0c060] focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#f0c060]"
        >
