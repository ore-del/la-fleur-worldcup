    ...
    <>
      ...
      <div className="fixed inset-0 z-[1]">
        <picture className="w-full h-full">
          <source media="(max-width: 767px)" srcSet="/hero-bkg.png" />
          <img src="/hero-bkg.png" alt="" aria-hidden className="w-full h-full object-cover object-[30%_center]" />
        </picture>
      ...
      <main className="min-h-screen pt-[88px]">
        <div className="relative z-[2] max-w-[1184px] mx-auto px-6 py-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-9">
            ...
            {/* Standard */}
            <div className="rounded-[16px] border border-[rgba(59,112,70,0.25)] bg-[rgba(42,43,44,0.82)] backdrop-blur-[20px]">
              <div className="p-[30px] flex flex-col">
                <p className="text-white font-bold text-[24px] mb-2">{p.standardName}</p>
                <div className="flex items-baseline gap-3 mb-1">
                  <span className="text-[#6db87e] font-bold text-[52px] tracking-[-2px] leading-none">$249</span>
                </div>
                <p className="text-white/40 text-[13px] mb-5">{p.standardBilling}</p>
                <div className="h-px bg-[rgba(59,112,70,0.12)] mb-5" />
                ...
                <div className="flex flex-col gap-[14px] mb-8">
                  {p.standardFeatures.map((f, i) => (
                    <div key={i} className="flex items-start gap-4">
                      <span className="text-[#6db87e] font-bold text-[13px] leading-[1.5] shrink-0">✓</span>
                      <span className="text-white/80 text-[14px]">{f}</span>
                    </div>
                  ))}
                </div>
                <button onClick={() => openModal(0)}
                  className="h-[52px] w-full rounded-full bg-black text-white font-semibold text-[15px] hover:bg-white hover:text-[#080603] transition-colors duration-200">
                  {p.standardCta}
                </button>
              </div>
            </div>
            ...
            {/* Premium */}
            <div className="rounded-[16px] border-2 border-[rgba(59,112,70,0.5)] bg-[rgba(42,43,44,0.82)] backdrop-blur-[20px] overflow-hidden">
              <div className="h-[40px] bg-[#3b7046] flex items-center justify-center">
                <span className="text-white font-semibold text-[13px]">{p.premiumBanner}</span>
              </div>
              <div className="p-[30px] flex flex-col">
                <p className="text-white font-bold text-[24px] mb-3">{p.premiumName}</p>
                <div className="flex flex-col gap-1 mb-2">
                  <div className="flex items-baseline gap-2">
                    <span className="text-[#6db87e] font-bold text-[48px] tracking-[-2px] leading-none">$699</span>
                ...
                <div className="h-px bg-[rgba(59,112,70,0.18)] mb-5" />
                ...
                <button onClick={() => openModal(1)}
                  className="h-[52px] w-full rounded-full bg-[#3b7046] hover:bg-[#6db87e] text-white font-semibold text-[15px] transition-colors duration-200">
                  {p.premiumCta}
                </button>
              </div>
            </div>
          ...
          {/* Timeline teaser */}
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-between gap-4 rounded-[16px] border border-[rgba(59,112,70,0.15)] bg-[rgba(42,43,44,0.6)] backdrop-blur-[20px] px-7 py-5">
            <div>
